...
folder are mapped to the application's root namespace, 'xcpdevtools'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "xcpdevtools"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'xcpdevtools' in the same way
as `"xcpdevtools/sass/src"`.
